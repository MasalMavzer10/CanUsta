...
const CFG = {
  /* --- rendering ------------------------------------------------------- */
  VIEW_W: 480,              // internal resolution, true 16:9 retro
  VIEW_H: 270,
...
class Input {
  constructor(target) {
    ...
    window.addEventListener('pointerdown', (e) => {
      if (e.pointerType === 'touch') document.documentElement.classList.add('touch-enabled');
    }, { passive: true });
  }
...
class Kitchen {
  ...
  _use() {
    ...
    switch (s.kind) {
      /* ---- pick up a base ------------------------------------------- */
      case 'base': {
        if (this.carried) { this.say('ELİN DOLU - S+SPACE İLE AT'); return; }
  ...
  _promptFor(s) {
    switch (s.kind) {
      case 'base':
        ...
        return this.carried ? null : 'SPACE: AL';
      case 'season': return this.carried && this.carried.raw ? 'SPACE: BAHARATLA' : null;
      case 'grill':
        if (this.carried && this.carried.raw && s.slots.length < this.grillCapacity) return 'SPACE: MANGALA KOY';
        if (!this.carried && s.slots.length) return 'SPACE: MANGALDAN AL';
        return null;
      case 'pass':
        if (this.carried && this.front) return 'SPACE: SERVİS ET';
        if (!this.carried && this.shelf.length) return 'SPACE: TABAĞI AL';
        return null;
      case 'shop': return 'SPACE: PERSONEL, EKİPMAN, HARİTA';
      case 'door':
        return this.carried ? null
             : this.lamb <= 0 ? 'SPACE: KASABA GİT!' : 'SPACE: KASABA GİT';
      case 'sink': return this.dirty > 0 && !this.carried ? 'SPACE BASILI TUT: BULAŞIK' : null;
    }
    return null;
  }
  ...
  _drawPayroll(ctx) {
    ...
    if (short > 0) {
      ...
      f.draw(ctx, 'SPACE: ELİNDEKİNİ ÖDE - GERİSİ DEFTERE',
             CFG.VIEW_W / 2, by, { align: 'center', color: '#f2b53c' });
      f.draw(ctx, 'X: SEÇİLENİ İŞTEN ÇIKAR',
             CFG.VIEW_W / 2, by + 10, { align: 'center', color: '#c9bda8' });
    } else {
      f.draw(ctx, 'SPACE: MAAŞLARI ÖDE', CFG.VIEW_W / 2, by,
             { align: 'center', scale: 2, color: '#74e0b0', shadow: '#1b1220' });
      f.draw(ctx, 'X: SEÇİLENİ İŞTEN ÇIKAR',
             CFG.VIEW_W / 2, by + 18, { align: 'center', color: '#8d8296' });
    }
    f.draw(ctx, 'W S SEÇ', CFG.VIEW_W / 2, CFG.VIEW_H - 10,
  ...
  _drawButcher(ctx) {
    ...
    f.draw(ctx, 'W S SEÇ     SPACE AL     P MUTFAĞA DÖN',
  ...
  _drawShop(ctx) {
    ...
    f.draw(ctx, this.closed
             ? `A D SEKME   W S SEÇ   SPACE AL   P: ${this.day + 1}. GÜNÜ AÇ`
  ...
  _drawDebtTab(ctx, top) {
    ...
    f.draw(ctx, 'SPACE: SEÇİLİ BORCU ÖDE', CFG.VIEW_W - 32, ty + 5,
           { align: 'right', color: '#8d8296' });
  }
  ...
  _drawMapTab(ctx, top) {
    ...
    if (this.branches >= 2) {
      ...
      f.draw(ctx, `SPACE: ${BRANCHES[other].name} ŞUBESİNE GİT`, 34, y2 + 4,
...
const GSTATE = {
  ...
  PAUSED: 'paused',
  WON: 'won',               // ACT 1 cleared -> bridges into ACT 2
  LOST: 'lost',
...
class Game {
  ...
  /** CSS fills the visual viewport; the backing store stays crisp at 480x270. */
  _resize() {
    const viewport = window.visualViewport;
    this.canvas.style.width = `${Math.round(viewport?.width || window.innerWidth)}px`;
    this.canvas.style.height = `${Math.round(viewport?.height || window.innerHeight)}px`;
  }
  ...
  _updateMobileAction() {
    ...
    let label = 'EYLEM';
    let sub = 'DOKUN';
    if (this.state === GSTATE.TITLE) { label = 'BAŞLA'; sub = 'ONAYLA'; }
    else if (this.state === GSTATE.PLAYING) { label = 'YAKALA'; sub = 'KOYUNU AĞILA GÖTÜR'; }
    else if (this.state === GSTATE.WON || this.state === GSTATE.KITCHEN_INTRO) {
      label = 'DEVAM'; sub = 'MANGALA GEÇ';
    } else if (this.state === GSTATE.LOST || this.state === GSTATE.KITCHEN_LOST) {
      label = 'TEKRAR'; sub = 'O';
    } else if (this.state === GSTATE.PAUSED) { label = 'DEVAM'; sub = 'P'; }
    else if (this.state === GSTATE.KITCHEN && this.kitchen) {
      label = this.kitchen.contextActionLabel(); sub = 'EYLEM';
    } else if (this.state === GSTATE.ENDING) { label = 'ATLA'; sub = 'DEVAM'; }
  ...
  _drawTitle(ctx) {
    ...
    f.draw(ctx, save ? 'W S SEÇ     E ONAYLA' : 'E İLE BAŞLA',
           CFG.VIEW_W / 2, 206, { align: 'center', color: '#8d8296' });
    f.draw(ctx, `A D İLE MOD SEÇ: ${DIFFICULTY_LABEL[this.difficulty]}`,
           CFG.VIEW_W / 2, 218,
           { align: 'center', color: this.difficulty === DIFFICULTY.RELAXED ? '#74e0b0' : '#f2b53c',
             shadow: '#1b1220' });
    f.draw(ctx, 'A D YÜRÜ   W ZIPLA   S EĞİL   SPACE YAKALA   E KULLAN',
  ...
  _drawPaused(ctx) {
    this.hud.banner(ctx, [
      ...
      { text: 'P DEVAM   O BAŞTAN', scale: 1, color: '#c9bda8' },
    ], { y: 108 });
  }
  ...
  _drawWin(ctx) {
    this.hud.banner(ctx, [
      ...
      { text: this.winTimer > 1.2 ? 'SPACE - MANGAL YANDI' : ' ',
        scale: 2, color: '#f2b53c', blink: true, rate: 6, gap: 12 },
      { text: 'O İLE BİRİNCİ PERDE', scale: 1, color: '#8d8296' },
  ...
  _drawKitchenIntro(ctx) {
    ...
    f.draw(ctx, 'SPACE TEZGAH      S + SPACE TABAĞI AT',
           CFG.VIEW_W / 2, 196, { align: 'center', color: '#8d8296' });
    if (Math.sin(this.time * 6) > -0.3) {
      f.draw(ctx, 'AÇMAK İÇİN SPACE', CFG.VIEW_W / 2, 216,
             { align: 'center', scale: 2, color: '#74e0b0', shadow: '#1b1220' });
    }
  }
  ...
  _drawKitchenWon(ctx) {
    ...
    this.hud.banner(ctx, [
      ...
      { text: 'BAŞTAN OYNAMAK İÇİN O', scale: 1, color: '#8d8296', gap: 14 },
  ...
  _drawKitchenLost(ctx) {
    ...
    this.hud.banner(ctx, [
      ...
      { text: 'TEKRAR İÇİN O.', scale: 2, color: '#f8e9cf', blink: true, rate: 6, gap: 14 },
  ...
  _drawLose(ctx) {
    this.hud.banner(ctx, [
      ...
      { text: 'TEKRAR İÇİN O.', scale: 2, color: '#f8e9cf', blink: true, rate: 6 },
